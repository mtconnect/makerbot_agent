/**
 * ibutton crc: Calculate the CRC of some known strings, in order to generate reference cases for other CRC implementations
 *
 * CRC implementation taken from ReplicatorG
 */
...
void setup() {
  ...
  for(int i = 0; i < test_inputs.length; i++) {
    println(i + ": " + Integer.toHexString((char)CalculateCRC(test_inputs[i])));
  }
}
